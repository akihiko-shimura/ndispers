...
二軸性結晶は誘電主平面ごとに 1 クラスを提供します。角度引数はその面で変化する方（xy では φ、yz と zx では θ）で、インスタンスの `theta_rad` / `phi_rad` 属性がどちらかを示します。Sellmeier 方程式に温度項のない媒質（α-BBO、方解石、サファイア、水晶、5% MgO:LiNbO₃）もシグネチャ統一のため温度引数を取りますが無視し、`dndT` は 0 を返します。
