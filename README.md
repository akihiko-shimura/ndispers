...
where arguments are wavelength (in micro-meter), temperature (in deg.C), theta (in radians) and polarization (`pol='o' or 'e'`) as an option ('o' as default). Note that `pol='e'` corresponds to `pol='o'` for theta angle is 0 radians.

Output value is generically of `numpy.arrray` type. You can input an array to each arguments, getting an output array of the same shape.
