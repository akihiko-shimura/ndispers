...
At θ = 0 (propagation along the optic axis) the o-ray and e-ray indices
coincide. Scalar input returns a float; numpy-array input broadcasts and
returns an array:
...
The other dispersion methods (`GD`, `GV`, `ng`, `GVD`, `TOD`, `GVM`,
...
Optically isotropic media — the glasses and the cubic crystals — take only
...
`help(bbo)` (or `bbo?` in IPython/Jupyter) prints the material data, the
Sellmeier equation and the references; `bbo.constants` returns the
coefficient values.
...
[Conventions](https://ndispers.readthedocs.io/en/latest/conventions/) page.
...
[validation page](https://ndispers.readthedocs.io/en/latest/validation/)
