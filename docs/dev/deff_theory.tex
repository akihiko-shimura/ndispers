...
equation onto the code, records where the v0.9.1 implementation departed
from the theory, and describes the generic contraction used from v0.10. The same recipe extends to $\bar{4}2m$, $4mm$,
...
Two further defects surfaced while collecting reference values: the SLN
class held $d_{22}=25$\,pm/V, which is LiNbO$_3$'s $d_{33}$ (its $d_{22}$
is $2.1$\,pm/V \cite{Roberts1992}); and the zx principal-plane classes of
LBO and KTP declared $\phi=\pi/2$ (that is the yz plane; zx is $\phi=0$).
Neither affected refractive indices, since $n$ in those planes does not
depend on $\phi$, but both would have entered $\ueff$.

\subsection{What v0.10 does}

Rather than transcribing a closed form per point group and interaction
type, \texttt{NonlinearGroup.}\allowbreak\texttt{deff\_sfg} evaluates
Eq.~\eqref{eq:deff_def} directly: the point-group class lists the nonzero
tensor entries of each independent component with Kleinman symmetry
applied (one line per matrix cell of Eqs.~\eqref{eq:d3m},
\eqref{eq:d32} and their analogues), the crystal lists one reference
measurement per component, and the field vectors are generated from
Eqs.~\eqref{eq:eo}--\eqref{eq:ee}. For a uniaxial crystal or a principal
plane of a biaxial one, both vectors follow from the wavevector direction
$\vecu k(\alpha)$ as a function of the varying angle $\alpha$ ($\theta$,
or $\phi$ in the xy plane): $\vecu e_e=\partial_\alpha\vecu k$ evaluated at
$\alpha+\rho$, and $\vecu e_o=\vecu k\times\partial_\alpha\vecu k$. This
reproduces Eqs.~\eqref{eq:eo}--\eqref{eq:ee} for uniaxial crystals and the
standard vectors (o-wave along the plane normal, e-wave in the plane) for
the biaxial planes, with one sign convention for all waves, so the
relative signs of terms in type-II expressions are fixed.

Miller's rule is evaluated with the principal $\chi$'s of
Eq.~\eqref{eq:chi}: $n_o$ and $n_e(\theta{=}\pi/2)$ for uniaxial crystals,
$n_x,n_y,n_z$ from the crystal's principal Sellmeier expressions for
biaxial ones. The closed forms are kept as tests
(\texttt{tests/test\_nonlinear.py}), which compare the contraction with
\cite{Dmitriev}'s expressions to $10^{-12}$:
\begin{align}
\bar 42m:&\quad \ueff^{\rm ooe}=d_{36}\sin\Theta_3\sin2\phi,\qquad
\ueff^{\rm oee}=d_{36}\sin(\Theta_2+\Theta_3)\cos2\phi;\\
4mm:&\quad \ueff^{\rm ooe}=d_{31}\sin\Theta_3,\qquad \ueff^{\rm oee}=0;\\
\text{LBO}:&\quad \ueff^{\rm ooe}(xy)=d_{32}\cos\Phi_3,\qquad
\ueff^{\rm oeo}(yz)=d_{31}\cos\Theta_2,\notag\\
&\quad \ueff^{\rm eeo}(zx)=d_{31}\cos\Theta_1\cos\Theta_2+d_{32}\sin\Theta_1\sin\Theta_2,\notag\\
&\quad \ueff^{\rm oee}(zx)=d_{31}\cos\Theta_2\cos\Theta_3+d_{32}\sin\Theta_2\sin\Theta_3;\\
\text{KTP}:&\quad \ueff^{\rm eoe}(xy)=d_{31}\sin\Phi_1\sin\Phi_3+d_{32}\cos\Phi_1\cos\Phi_3,\notag\\
&\quad \ueff^{\rm oeo}(yz)=d_{31}\sin\Theta_2,\qquad \ueff^{\rm oeo}(zx)=d_{32}\sin\Theta_2,
\end{align}
(LBO: $x,y,z=a,-c,b$; KTP: $x,y,z=a,b,c$; both class $mm2$)
with $\Phi_m=\phi+\rho_m$ in the xy plane. For LBO the component names
$d_{31},d_{32},d_{33}$ are those of the crystallographic frame $(a,b,c)$
with $c$ polar, as in the literature, while the dielectric frame is
$(a,-c,b)$; the class maps one to the other, and the sign of the axis
reversal is an overall sign of $\ueff$.

Reference values and sources, with the literature standing of Miller
scaling for each crystal, are recorded in each class's \texttt{\_d\_ref}
and \texttt{\_d\_note}: $\beta$-BBO \cite{Shoji1999}, KDP
\cite{Eckardt1990}, LiNbO$_3$/LiTaO$_3$/KTP \cite{Shoji1997}, LBO
\cite{Velsko1991,Roberts1992}, CLBO \cite{Mori1995}, KBBF and RBBF
\cite{ChenKBBF,ChenRBBF}, Li$_2$B$_4$O$_7$ \cite{Komatsu1997,Petrov1998}.
Signs follow \cite{Roberts1992} where he gives them ($d_{31}$ and $d_{32}$
of LBO opposite; $d_{22}$ and $d_{31}$ of BBO alike per
\cite{Alford2001}).
...
\bibitem{Shoji1999} I.~Shoji, H.~Nakamura, K.~Ohdaira, T.~Kondo, R.~Ito, T.~Okamoto, K.~Tatsuki, and S.~Kubota, ``Absolute measurement of second-order nonlinear-optical coefficients of $\beta$-BaB$_2$O$_4$ for visible to ultraviolet second-harmonic wavelengths,'' J. Opt. Soc. Am. B \textbf{16}, 620 (1999). \url{https://doi.org/10.1364/JOSAB.16.000620}

\bibitem{Eckardt1990} R.~C. Eckardt, H.~Masuda, Y.~X. Fan, and R.~L. Byer, ``Absolute and relative nonlinear optical coefficients of KDP, KD*P, BaB$_2$O$_4$, LiIO$_3$, MgO:LiNbO$_3$, and KTP measured by phase-matched second-harmonic generation,'' IEEE J. Quantum Electron. \textbf{26}, 922 (1990). \url{https://doi.org/10.1109/3.55534}

\bibitem{Velsko1991} S.~P. Velsko, M.~Webb, L.~Davis, and C.~Huang, ``Phase-matched harmonic generation in lithium triborate (LBO),'' IEEE J. Quantum Electron. \textbf{27}, 2182 (1991). \url{https://doi.org/10.1109/3.135177}

\bibitem{Mori1995} Y.~Mori, I.~Kuroda, S.~Nakajima, T.~Sasaki, and S.~Nakai, ``Nonlinear optical properties of cesium lithium borate,'' Jpn. J. Appl. Phys. \textbf{34}, L296 (1995). \url{https://doi.org/10.1143/JJAP.34.L296}

\bibitem{ChenKBBF} C.~Chen, G.~Wang, X.~Wang, and Z.~Xu, ``Deep-UV nonlinear optical crystal KBe$_2$BO$_3$F$_2$---discovery, growth, optical properties and applications,'' Appl. Phys. B \textbf{97}, 9 (2009). \url{https://doi.org/10.1007/s00340-009-3554-4}

\bibitem{ChenRBBF} C.~Chen, Y.~Wu, Y.~Li, \emph{et al.}, ``Deep UV nonlinear optical crystal: RbBe$_2$(BO$_3$)F$_2$,'' J. Opt. Soc. Am. B \textbf{26}, 1519 (2009). \url{https://doi.org/10.1364/JOSAB.26.001519}

\bibitem{Komatsu1997} R.~Komatsu, T.~Sugawara, K.~Sassa, \emph{et al.}, ``Growth and ultraviolet application of Li$_2$B$_4$O$_7$ crystals: Generation of the fourth and fifth harmonics of Nd:Y$_3$Al$_5$O$_{12}$ lasers,'' Appl. Phys. Lett. \textbf{70}, 3492 (1997). \url{https://doi.org/10.1063/1.119210}

\bibitem{Petrov1998} V.~Petrov, F.~Rotermund, F.~Noack, R.~Komatsu, T.~Sugawara, and S.~Uda, ``Vacuum ultraviolet application of Li$_2$B$_4$O$_7$ crystals: Generation of 100 fs pulses down to 170 nm,'' J. Appl. Phys. \textbf{84}, 5887 (1998). \url{https://doi.org/10.1063/1.368904}
